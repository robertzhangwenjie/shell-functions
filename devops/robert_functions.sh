...
del_noself_created_volumes() {
    docker volume ls | awk 'BEGIN{print "----------Volumes Listed Will Del----------"}/\w{50,100}/{print $2}END{print '\n'}'
    volumes=$(docker volume ls | awk '/\w{50,100}/{print $2}')
    echo -n "Are you ready to del all voluems listed, yes/no?"
    read flag
...
menus='del_noself_created_volumes exit'
